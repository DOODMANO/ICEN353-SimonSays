...
void showPattern()
{
  ...
  for(steps = 0; steps < sequence; steps++)
  {
    time = millis();
...
int verifyPattern()
{
    while(steps < sequence)
    {
      ...
      tone(piezzo, piezzoTones[buttonState - 8]);
      digitalWrite(buttonState, HIGH);
      delay(500);
      noTone(piezzo);
      digitalWrite(buttonState, LOW);
      if(ledPattern[steps] == buttonState)
      {
      ...
      {
        Serial.println("FAILED! Wrong button");
        return 0;
      }
      }
    }
    Serial.println("Added step to new sequence");
    return 1;
}
